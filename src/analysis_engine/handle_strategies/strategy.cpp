...
json CycleStrategy::report(const std::string &start_address) const {
  json data;
...
FanInBFS(EthGraph *graph, NodeId start, size_t max_depth, NodeId main_wallet) {
  ...
  while (!queue.empty()) {
    ...
    for (const auto &neighbor : graph->get_nodes_from(current)) {
      if (neighbor == main_wallet || neighbor == start) {
        continue;
      }
...
json FanInStrategy::report(const std::string &start_address) const {
  ...
  for (const auto &neighbor : neighbors) {
    futures.push_back(std::async(std::launch::async, FanInBFS, graph, neighbor,
                                 deep, start_id));
  }
...
std::unordered_set<NodeId> FanInFanOutBFS(EthGraph *graph, NodeId start,
  ...
  while (!queue.empty()) {
    ...
    for (NodeId neighbor : graph->get_nodes_from(current)) {
      if (neighbor == main_wallet || neighbor == start) {
...
json FanInFanOutStrategy::report(const std::string &start_address) const {
  NodeId start_id = graph->get_node_id(start_address);
  std::vector<NodeId> neighbors = graph->get_nodes_from(start_id);
...
json TransitStrategy::report(const std::string &start_address) const {
  ...
  const std::vector<uint64_t> &timestamp_vec_ref = graph->get_edges_timestamp();
...
json RatioStrategy::report(const std::string &start_address) const {
  auto node_id = graph->get_node_id(start_address);
  auto edges_from_id = graph->get_edges_from(node_id);
  auto edges_to_id = graph->get_edges_to(node_id);
  ...
  if (ratio >= 0.90 && ratio <= 1.05) {
    ...
    data["res_string"] =
        "Address " + start_address +
        " looks like a normal user (Net Flow Ratio: " + std::to_string(ratio) +
